...
export function Exercise() {
  ...
  function isEmailValid(email) {
    ...
    // }
    // Porównanie
    // returne email? !email? true : false : false;
  ...
  if (isEmailValid(0)) {
    console.log('Email jest nieprawidłowy');
  }
  ...
  const content = (
    <>
      ...
      {!isLoading && isUserLoggedIn && (
        <div>Kontent widoczny dla zalogowanych użytkowników</div>
      )}
      {isAdmin && <div>Kontent widoczny dla administratora</div>}
      {isPremiumAndLoggedIn && (
        <div> Kontent widoczny dla użytkownika premium</div>
      )}
    </>
  );
  //  Operatorzy
  ...
  //  ||= OR ASSIGN
  let x = 5;
  x ||= 10; // x nadal będzie równe 5 bo 5 nie jest wartości false
  console.log(x, 'x ||= 10');
  let y = 0;
  y ||= 10; // y nadal będzie równe 10 bo 0 jest wartością false
  console.log(y, 'y ||=10');

  //  &&= AND ASSIGN
  let z = 5;
  z &&= 10; // z będzie równe 10 bo 5 jest wartocią true
  console.log(z, 'z &&= 10');
  let w = 0;
  ...
  // 1. () Nawiasy (Najwyższy piorytet - wyrażenia w nawiasach są wykonywane jako pierwsze).
  // 2. ** Potęgowanie.
